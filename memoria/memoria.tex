...
Inicialmente se nos plantea un problema de regresión, cuyo objetivo es predecir la popularidad (contada en número de artículos compartidos en redes sociales) a partir de las características sobre los artículos publicados. Sin embargo, siguiendo las recomendaciones de los creadores del dataset trataremos este problema como un problema de clasificación binario, considerando todos aquellos valores menores e iguales que $1400$ \textit{shares} como una clase y los mayores como la otra.

El dataset consta de 60 atributos, dos de ellos no predictivos (\textit{url} y \textit{timedelta}), por lo tanto nuestro vector de características tendrá una longitud de 58. Por lo tanto identificamos los datos del problema como:
\begin{itemize}
\item $X: \mathbb{R}^{58}$
\item $Y: \{-1,1\}$
